...
def check_constraints(q, epsilon, sensitivity, index, y=0.5):
    # Check parameter
...
def obtain_Ex(q, epsilon, sensitivity, index, y=0.5):
    t = q * epsilon / sensitivity
    L = sensitivity
    k = obtain_k(L, t, index, y)
...
def obtain_Var(q, epsilon, sensitivity, index, y=0.5):
    t = q * epsilon / sensitivity
    L = sensitivity
    k = obtain_k(L, t, index, y)
...
def parameter_tuning(epsilon, sensitivity, index):
    q_ini = 0.01
    step = 0.01
    ...
    if (index == 1):
        ...
        return q_final, y_final, Var_final
...
def probability_fun(x, k, t, epsilon, sensitivity, index, y=0.5):
    L = sensitivity
...
def generate_noise_optimized(q, epsilon, sensitivity, index, y=0.5):
    L = sensitivity
...
#         else:
#             k = obtain_k(L, t, index)
#         P_x = probability_fun(x_count, k, t, epsilon, sensitivity, index, y if index == 1 else None)
#         rp = int(P_x * 1000)
#         for _ in range(rp):
#             yield x_count
#         x_count += step

def perturbation(fd, epsilon, sensitivity, index):
    q = 0
    y = 0

    if (index == 1):
        q, y, Var = parameter_tuning(epsilon, sensitivity, index)
        noise_array = generate_noise_optimized(q, epsilon, sensitivity, index, y)
        perturbed_result = fd + noise_array[0]

        Var = obtain_Var(q, epsilon, sensitivity, index, y)
        return perturbed_result, Var

    else:
        q, Var = parameter_tuning(epsilon, sensitivity, index)
        noise_array = generate_noise_optimized(q, epsilon, sensitivity, index)
        perturbed_result = fd + noise_array[0]

        Var = obtain_Var(q, epsilon, sensitivity, index)
        return perturbed_result, Var


def perturbation_Q(fd, q, epsilon, sensitivity, index, y=0.5):
    noise_array = generate_noise_optimized(q, epsilon, sensitivity, index, y)
    perturbed_result = fd + noise_array[0]
    return perturbed_result


def perturbation_Q_multiple(fd, q, epsilon, sensitivity, index, volume, y=0.5):
    perturbed_result = []
    noise_array = generate_noise_optimized(q, epsilon, sensitivity, index, y)
...
def perturbation_Q_multiple_list(fd, q, epsilon, sensitivity, index, y=0.5):
    perturbed_result = []
    noise_array = generate_noise_optimized(q, epsilon, sensitivity, index, y)
    bias = obtain_Ex(q, epsilon, sensitivity, index, y)
...
# def perturbation_Q_multiple(fd, q, epsilon, sensitivity, index, volume, y=0.5):
#     perturbed_result = []
#     noise_generator = generate_noise_optimized(q, epsilon, sensitivity, index, y)
#     bias = obtain_Ex(q, epsilon, sensitivity, index, y)
#     for _ in range(volume):
#         try:
#             noise_value = next(noise_generator) - bias
#         except StopIteration:
#             # 如果生成器耗尽，抛出异常或者重新开始，取决于您的需求
#             raise ValueError("Noise generator exhausted before reaching the required volume.")
#         perturbed_result.append(fd + noise_value)
#     return perturbed_result
